...
class EruditeBot(RBCAgentCore):
    ...
    def sense_strategy(self, 
        ...
        board_data = generate_input_for_model(board, capture_square, sense_history)
        input = torch.from_numpy(board_data)

        model_path = 'C:\\Users\\aniss\\OneDrive\\Dokumente\\Uni\\Semester4\\RBC\\RBC-Agent\\models\\sl-model\\top3-partial.pth'
        ...
        _ , predicted_output = torch.max(model(input) , 1) 

        flip = our_color == chess.BLACK
        sense = revert_sense_square(predicted_output.item(), flip=flip)
